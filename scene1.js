class Scene1 extends Phaser.Scene {
  ...
  create() {
    ...
    const rectangle1 = [this.add.grid(-1000, -1000, 200, 50, 50, 50, 0x00bfff), 0x00bfff, []]; //shapes can't be rotated afaik 
    this.physics.add.existing(rectangle1[0]);	                                                 //(can rotate the shape's image but not its body)
    const rectangle2 = [this.add.grid(-1000, -1000, 50, 200, 50, 50, 0x00bfff), 0x00bfff, []]; //so I made both horizontal and vertical versions
    ...
    function swapToNewShape(/*notThisShape*/) { //picks a random shape to fall down from the top and keeps other shapes outside the grid 
      /*let options, i, n;
      switch (notThisShape) { //if you don't want the same shape twice in a row
        case 'square':
        options = [2, 3, 4, 5];
    ...
    //RESTART
    this.input.keyboard.on('keyup-' + 'R', () => {this.scene.restart();});

    //FOR BEING ABLE TO ALLIGN THE SHAPES NEATLY WHEN AN OVERLAP HAPPENS
    let oddShapeY = -24;
    for (let i = 0;i<16;i++) {
    ...
    const addOverlap = (shape1, shape2) => {
      this.physics.add.overlap(shape1[0], clones, () => {
        if (!stopOverlap) { 
          stopOverlap = true; //avoid triggering multiple overlaps when hitting multiple clones etc
          if (shape1 === triangle12b && triangleState % 2 != 0) {shape2 = triangle1a;}
          else if (shape1 === triangle12b && triangleState % 2 === 0) {shape2 = triangle2a;}
          else if (shape1 === hammer12b && hammerState % 2 != 0) {shape2 = hammer1a;}
          else if (shape1 === hammer12b && hammerState % 2 === 0) {shape2 = hammer2a;}
          ...
          if ((currentShape === 'square' && shape1[0].y < 52) || //gameover conditions
            ...
            else {messageText.setText("Game Over");}
          }

          if (!gameState.gameOver) {
            score += 1;
            scoreText.setText(`Score: ${score}`);
            const shapeX = shape1[0].x;	//find the correct x and y for the clone shape that will be placed when you hit something
            function closeY(value) {return Math.abs(shape1[0].y - value) < 25;} 
            const shapeYArr = shape1[2].filter(closeY);
            const shapeY = shapeYArr[0];
            shape1[0].x = -1000; shape1[0].y = -1000; //move the shape out of sight 
            if (shape2 != undefined) {shape2[0].x = -1000; shape2[0].y = -1000;}
            
            const addClone = (x1, y1, x2, y2, x3, y3, x4, y4, color) => { //will leave 4 small blocks in the shape of the shape (given the right coordinates)
            ...
            if (shape1 === square) {addClone(-25, -25, -25, 25, 25, -25, 25, 25, shape1[1]);}
            else if (shape1 === rectangle1) {addClone(-25, 0, -75, 0, 25, 0, 75, 0, shape1[1]);}
            else if (shape1 === rectangle2) {addClone(0, -25, 0, -75, 0, 25, 0, 75, shape1[1]);}
            else if (shape1 === triangle1a) {
              if (triangleState === 1) {addClone(0, -50, 0, 0, 0, 50, 50, 0, shape1[1]);}
              else if (triangleState === 3) {addClone(0, -50, 0, 0, 0, 50, -50, 0, shape1[1]);}
            }
            else if (shape1 === triangle2a) {
              if (triangleState === 2) {addClone(-50, 0, 0, 0, 50, 0, 0, 50, shape1[1]);}
              else if (triangleState === 4) {addClone(-50, 0, 0, 0, 50, 0, 0, -50, shape1[1]);}
            }
            else if (shape1 === triangle12b) {
              if (triangleState === 1) {addClone(0, 0, -50, -50, -50, 0, -50, 50, shape1[1]);}
              else if (triangleState === 2) {addClone(0, 0, -50, -50, 0, -50, 50, -50, shape1[1]);}
              else if (triangleState === 3) {addClone(0, 0, 50, -50, 50, 0, 50, 50, shape1[1]);}
              else if (triangleState === 4) {addClone(0, 0, -50, 50, 0, 50, 50, 50, shape1[1]);}
            }
            else if (shape1 === hammer1a) {
              if (hammerState === 1) {addClone(0, -50, 0, 0, 0, 50, 50, -50, shape1[1]);}
              else if (hammerState === 3) {addClone(0, -50, 0, 0, 0, 50, -50, 50, shape1[1]);}
            }
            else if (shape1 === hammer2a) {
              if (hammerState === 2) {addClone(-50, 0, 0, 0, 50, 0, 50, 50, shape1[1]);}
              else if (hammerState === 4) {addClone(-50, 0, 0, 0, 50, 0, -50, -50, shape1[1]);}
            }
            else if (shape1 === hammer12b) {
              if (hammerState === 1) {addClone(0, 0, -50, 0, -50, 50, -50, 100, shape1[1]);}
              else if (hammerState === 2) {addClone(0, 0, 0, -50, -50, -50, -100, -50, shape1[1]);}
              else if (hammerState === 3) {addClone(0, 0, 50, 0, 50, -50, 50, -100, shape1[1]);}
              else if (hammerState === 4) {addClone(0, 0, 0, 50, 50, 50, 100, 50, shape1[1]);}
            }
            else if (shape1 === zigzag1a) {
              if (zigzagState === 1) {addClone(0, 25, 0, -25, 50, -25, 50, -75, shape1[1]);}
              else if (zigzagState === 3) {addClone(0, 25, 0, -25, -50, 25, -50, 75, shape1[1]);}
            }
              ...
              if (zigzagState === 2) {addClone(25, 0, -25, 0, 25, 50, 75, 50, shape1[1]);}
              else if (zigzagState === 4) {addClone(25, 0, -25, 0, -25, -50, -75, -50, shape1[1]);}
            }
    ...
    function shapeAssigner() {
      let shape1, shape2;
      if (currentShape === 'square') {shape1 = square[0]; shape2 = undefined;}
      else if (currentShape === 'rectangle') {
        shape2 = undefined;
        if (rectangleState % 2 != 0) {shape1 = rectangle1[0];}
        else {shape1 = rectangle2[0];}
      }
      else if (currentShape === 'triangle') {
        shape2 = triangle12b[0];
        if (triangleState % 2 != 0) {shape1 = triangle1a[0];}
        else {shape1 = triangle2a[0];}
      }
      else if (currentShape === 'hammer') {
        shape2 = hammer12b[0];
        if (hammerState % 2 != 0) {shape1 = hammer1a[0];}
        else {shape1 = hammer2a[0];}
      }
      else if (currentShape === 'zigzag') {
        if (zigzagState % 2 != 0) {shape1 = zigzag1a[0]; shape2 = zigzag1b[0];}
        else {shape1 = zigzag2a[0]; shape2 = zigzag2b[0];}
      }
      return [shape1, shape2];
    }

    //TURNING STOPOVERLAP OFF WHEN HITTING CLONE BELOW //I only want overlaps to happen when hitting something from above
    function overlappingBelow() { //because then you can allow a little bit of top-overlap to let the player fill a gap from the side
    ...
    this.input.keyboard.on('keyup-' + 'SPACE', () => {
      function shootDown() {
        const shape1 = shapeAssigner()[0]; const shape2 = shapeAssigner()[1];
        let distance1; let distance2 = Infinity;
        const cloneDistances1 = []; const cloneDistances2 = [];
        const eligibleClones1 = []; const eligibleClones2 = [];
    ...
    const tooCloseToCloneToRotate = (clone) => {
      ...
      if (currentShape === 'rectangle') {
        if (rectangleState % 2 != 0) {shape = rectangle1[0];}
        else {shape = rectangle2[0];}
        ...
        if (triangleState % 2 != 0) {shape = triangle1a[0];}
        else {shape = triangle2a[0];}
        const distanceX = Math.abs(clone.x - shape.x);
        const minDistanceX1 = (clone.width + shape.width)/2 + 50;
        const distanceY = Math.abs(clone.y - shape.y);
        const minDistanceY1 = (clone.height + shape.height)/2 + 50;
        ...
        if (hammerState % 2 != 0) {shape = hammer1a[0];}
        else {shape = hammer2a[0];}
        const distanceX = Math.abs(clone.x - shape.x);
        const minDistanceX1 = (clone.width + shape.width)/2 + 50;
        const distanceY = Math.abs(clone.y - shape.y);
        const minDistanceY1 = (clone.height + shape.height)/2 + 50;
        ...
        if (zigzagState % 2 != 0) {shape = zigzag1a[0];}
        else {shape = zigzag2a[0];}
        const distanceX = Math.abs(clone.x - shape.x);
        const minDistanceX1 = (clone.width + shape.width)/2 + 50;
        const minDistanceX2 = (clone.width + shape.width)/2 + 100;
        const distanceY = Math.abs(clone.y - shape.y);
        ...
        if (zigzagState === 1) {
          ...
          else if (shape.y < clone.y) {b = false;}
          return a || b; 
        }
          ...
          else if (shape.x < clone.x) {a = false;}
          if (shape.y > clone.y) {b = false;}
    ...
    //REMOVING FULL LINES
    const yArrays = []; let arrayHeight = 726;
    for (let i=0;i<14;i++) {
      yArrays.push([arrayHeight]);
      arrayHeight -= 50;
    }
